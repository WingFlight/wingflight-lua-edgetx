...
local function create(zone, options)
    return loadScript("/WIDGETS/WfTool/app.lua")(zone, options)
end

local function update(widget, options)
    if widget.update then
        wf.call(widget.update, widget, options)
    end
end

local function refresh(widget, event, touchState)
    if widget.refresh then
        wf.call(widget.refresh, widget, event, touchState)
    end
end

local function background(widget)
    if widget.background then
        wf.call(widget.background, widget)
    end
end

-- local function translate(widget)
--     --print("WfTool: translate called")
