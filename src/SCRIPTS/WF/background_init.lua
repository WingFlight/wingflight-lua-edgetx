...
local function setModelName(name)
    ...
    --local newName = ">" .. ((name and #name > 0) and name or "WingFlight")
    local newName = name
...
local function waitForCrsfSensorsDiscovery()
    ...
    if not sensorsDiscovered then
        ...
        sensorsDiscoveredTimeout = wf.clock() + 2
    end

    if sensorsDiscoveredTimeout ~= 0 then
        if wf.clock() < sensorsDiscoveredTimeout then
            return 1 -- wait for sensors to be discovered
        end
        sensorsDiscoveredTimeout = 0
    end

    --wf.print("Sensors already discovered")
    return 0
end
        ...
        function(_, version)
            ...
            if wf.apiVersion >= 12.07 then
                ...
                if crossfireTelemetryPush() then
                    wf.useApi("mspTelemetryConfig").getTelemetryConfig(
                        function(_, config)
            ...
            wf.useApi("mspSetRtc").setRtc(
                function()
                    if autoSetName then
                        setModelName(wf.modelName)
                    end
