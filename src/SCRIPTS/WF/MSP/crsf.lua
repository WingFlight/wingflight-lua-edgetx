-- Usage: local mspSend, mspPoll, maxTxBufferSize, maxRxBufferSize = wf.executeScript("MSP/csrf")
...
--[[
            wf.print("cmd:0x"..string.format("%X", cmd))
            wf.print("  data length: "..string.format("%u", #data))
            for i = 1,#data do
                wf.print("  ["..string.format("%u", i).."]:  0x"..string.format("%X", data[i]))
            end
--]]
            local mspData = {}
            for i = 3, #data do
