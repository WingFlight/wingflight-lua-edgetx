...
local function mspPoll()
    ...
    while true do
        ...
        if (sensorId == SMARTPORT_REMOTE_SENSOR_ID or sensorId == FPORT_REMOTE_SENSOR_ID) and frameId == REPLY_FRAME_ID then
            --wf.print("sensorId:0x"..string.format("%X", sensorId).." frameId:0x"..string.format("%X", frameId).." dataId:0x"..string.format("%X", dataId).." value:0x"..string.format("%X", value))
            local payload = {}
            ...
            --for i=1,#payload do
            --    wf.print(  "["..string.format("%u", i).."]:  0x"..string.format("%X", payload[i]))
            --end
            return payload
