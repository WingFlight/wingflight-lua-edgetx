...
local function applyProfile(data, profile)
    ...
    elseif profile.bec then
        ...
        data.bec_voltage.unit = wf.units.volt
    else
        data.bec_voltage.table = nil
...
local function getDefaults()
    return {
        ...
        bec_voltage = { min = 50, max = 84, scale = 10, unit = wf.units.volt },
        startup_time = { min = 4, max = 25, unit = wf.units.seconds },
        response_time = { min = 0, max = #responseTime, table = responseTime },
        gov_p_gain = { min = 0, max = 9 },
        gov_i_gain = { min = 0, max = 9 },
        auto_restart = { min = 0, max = 90, unit = wf.units.seconds },
        ...
        timing = { min = 0, max = 30, unit = wf.units.degrees },
...
local function getEscParameters(callback, callbackParam, data)
    ...
    local message = {
        ...
        processReply = function(self, buf)
            data.esc_signature.value = wf.mspHelper.readU8(buf)
            if data.esc_signature.value ~= 253 then
                return
            end
            data.command.value = wf.mspHelper.readU8(buf)
            data.firmware_version.value = wf.mspHelper.readText(buf, 16)
            data.hardware_version.value = wf.mspHelper.readText(buf, 16)
            data.esc_type2.value = wf.mspHelper.readText(buf, 16)
            data.esc_type.value = wf.mspHelper.readText(buf, 15)

            local itemBytes = {}
            for i = 1, ITEM_COUNT do
                itemBytes[i] = wf.mspHelper.readU8(buf)
            end
...
local function setEscParameters(data)
    ...
    wf.mspHelper.writeU8(message.payload, data.command.value)
    wf.mspHelper.writeText(message.payload, data.firmware_version.value or "", 16)
    wf.mspHelper.writeText(message.payload, data.hardware_version.value or "", 16)
    wf.mspHelper.writeText(message.payload, data.esc_type2.value or "", 16)
    wf.mspHelper.writeText(message.payload, data.esc_type.value or "", 15)
    for i = 1, ITEM_COUNT do
        wf.mspHelper.writeU8(message.payload, itemBytes[i] or 0)
    end
    wf.mspQueue:add(message)
end
