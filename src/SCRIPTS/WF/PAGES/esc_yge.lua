...
local escParameters = wf.useApi(mspEscYge).getDefaults()
...
return {
    read = function(self)
        wf.useApi(mspEscYge).read(receivedEscParameters, self, escParameters)
    end,
    write = function(self)
        wf.useApi(mspEscYge).write(escParameters)
