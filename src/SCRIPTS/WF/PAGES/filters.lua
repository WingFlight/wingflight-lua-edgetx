...
local labels = {}
local fields = {}
local filterConfig = wf.useApi("mspFilterConfig").getDefaults()
...
return {
    read = function(self)
        wf.useApi("mspFilterConfig").read(receivedFilterConfig, self, filterConfig)
    end,
    write = function(self)
        if filterConfig.gyro_lpf1_type.value then
            wf.useApi("mspFilterConfig").write(filterConfig)
