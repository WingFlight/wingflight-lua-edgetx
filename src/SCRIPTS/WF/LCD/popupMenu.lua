...
local function draw()
    local x = wf.radio.MenuBox.x
    local y = wf.radio.MenuBox.y
    local w = wf.radio.MenuBox.w
    local h_line = wf.radio.MenuBox.h_line
    local h_offset = wf.radio.MenuBox.h_offset
    ...
    for i = 1, #t.menu.items do
        ...
        lcd.drawText(x + wf.radio.MenuBox.x_offset, y + (i-1)*h_line + h_offset, item.text, textOptions)
    end
end
