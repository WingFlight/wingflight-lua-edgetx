...
local escParameters = wf.useApi(mspEscScorp).getDefaults()

local function rebootEsc(field, page)
    escParameters.command = 0x80
    wf.useApi(mspEscScorp).write(escParameters)
    wf.reloadPage()
end
...
return {
    read = function(self)
        wf.useApi(mspEscScorp).read(receivedEscParameters, self, escParameters)
    end,
    write = function(self)
        wf.useApi(mspEscScorp).write(escParameters)
