...
t.draw = function(pageState)
    ...
    local textOptions = wf.radio.textSize + lcdShared.textOptions
    local boldTextOptions = (wf.isEdgeTx and (TEXT_COLOR or COLOR_THEME_SECONDARY1) and BOLD + (TEXT_COLOR or COLOR_THEME_SECONDARY1)) or textOptions
...
local function requestPage()
    if not Page.reqTS or Page.reqTS + 5 <= wf.clock() then
        --wf.print("Requesting page...")
        Page.reqTS = wf.clock()
        if Page.read then
            Page:read()
        end
...
t.update = function(pageState, event)
    ...
    elseif pageState == lcdShared.pageStatus.editing then
        ...
        if event == EVT_VIRTUAL_INC or event == EVT_VIRTUAL_INC_REPT or event == EVT_VIRTUAL_DEC or event == EVT_VIRTUAL_DEC_REPT then
            local scrollSpeed = wf.clock() - scrollSpeedTS
            --wf.print(scrollSpeed)
            if scrollSpeed < 0.075 then
                scrollSpeedMultiplier = 5
            end
            scrollSpeedTS = wf.clock()
        end
