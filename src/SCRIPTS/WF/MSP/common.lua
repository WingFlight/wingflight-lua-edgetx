-- Usage: local mspSendRequest, mspProcessTxQ, mspPollReply, mspClearRxTxBufs = wf.executeScript("MSP/common")

local mspSeq = 0 -- Sequence number for next MSP packet
local mspRemoteSeq = 0
local mspRxBuf = {}
local mspRxError = false
local mspRxSize = 0
local mspRxCRC = 0
local mspRxReq = 0
local mspStarted = false
local mspLastReq = 0
local mspTxBuf = {}
local mspTxIdx = 1
local mspTxCRC = 0

local protocolScript = "MSP/" .. wf.executeScript("protocols")
local mspSend, mspPoll, telemetryPush, maxTxBufferSize, maxRxBufferSize = wf.executeScript(protocolScript)

local function mspProcessTxQ()
    if #mspTxBuf == 0 then
        return false
    end
    if not telemetryPush() then
        return true
    end
    --wf.print("Sending mspTxBuf size "..tostring(#mspTxBuf).." at Idx "..tostring(mspTxIdx).." for cmd: "..tostring(mspLastReq))
    local payload = {}
    local MSP_VERSION = bit32.lshift(mspLastReq > 255 and 2 or 1, 5)
    payload[1] = mspSeq + MSP_VERSION
    mspSeq = bit32.band(mspSeq + 1, 0x0F)
    if mspTxIdx == 1 then
        -- start flag
        local MSP_STARTFLAG = bit32.lshift(1,4)
        payload[1] = payload[1] + MSP_STARTFLAG
    end
    local i = 2
    while (i <= maxTxBufferSize) and mspTxIdx <= #mspTxBuf do
        payload[i] = mspTxBuf[mspTxIdx]
        mspTxIdx = mspTxIdx + 1
        mspTxCRC = bit32.bxor(mspTxCRC,payload[i])
        i = i + 1
    end
    if i <= maxTxBufferSize then
        payload[i] = mspTxCRC
        mspSend(payload)
        mspTxBuf = {}
        mspTxIdx = 1
        mspTxCRC = 0
        return false
    end
    mspSend(payload)
    return true
end

local function mspSendRequest(cmd, payload)
    --wf.print("Sending cmd "..cmd)
    -- busy
    if #mspTxBuf ~= 0 or not cmd then
        --wf.print("Existing mspTxBuf is still being sent, failed send of cmd: "..tostring(cmd))
        return nil
    end
    if cmd > 255 then -- MSP2
        mspTxBuf[#mspTxBuf + 1] = 0 -- flag
        mspTxBuf[#mspTxBuf + 1] = bit32.band(cmd, 0xFF)
        mspTxBuf[#mspTxBuf + 1] = bit32.rshift(cmd, 8)
        mspTxBuf[#mspTxBuf + 1] = bit32.band(#payload, 0xFF)
        mspTxBuf[#mspTxBuf + 1] = bit32.rshift(#payload, 8)
    else
        mspTxBuf[#mspTxBuf + 1] = #payload
        mspTxBuf[#mspTxBuf + 1] = bit32.band(cmd, 0xFF)
    end
    for i=1,#payload do
        mspTxBuf[#mspTxBuf + 1] = bit32.band(payload[i], 0xFF)
    end
    mspLastReq = cmd
    return mspProcessTxQ()
end

local function mspReceivedReply(payload)
    --wf.print("Starting mspReceivedReply")
    local idx = 1
    local status = payload[idx]
    local version = bit32.rshift(bit32.band(status, 0x60), 5)
    local start = bit32.btest(status, 0x10)
    local seq = bit32.band(status, 0x0F)
    idx = idx + 1
    --wf.print("payload length: "..#payload)
    --wf.print(" msp sequence #:  "..string.format("%u",seq))
    if start then
        -- start flag set
        mspRxBuf = {}
        mspRxError = bit32.btest(status, 0x80)
        if version == 0 then -- implicit request (no command ID)
            mspRxSize = payload[idx]
            mspRxReq = mspLastReq
            mspRxCRC = bit32.bxor(mspRxSize, mspRxReq)
            idx = idx + 1
        elseif version == 1 then
            mspRxSize = payload[idx]
            mspRxReq = payload[idx+1]
            mspRxCRC = bit32.bxor(mspRxSize, mspRxReq)
            idx = idx + 2
        elseif version == 2 then
            idx = idx + 1 -- skip flag
            mspRxReq = payload[idx] + bit32.lshift(payload[idx+1], 8)
            mspRxSize = payload[idx+2] + bit32.lshift(payload[idx+3], 8)
            idx = idx + 4
        end
        if mspRxReq == mspLastReq then
            mspStarted = true
            --wf.print("Started cmd "..mspLastReq)
        end
    elseif not mspStarted then
        --wf.print("  mspReceivedReply: missing Start flag")
        return nil
    elseif bit32.band(mspRemoteSeq + 1, 0x0F) ~= seq then
        mspStarted = false
        return nil
    end
    while (idx <= maxRxBufferSize) and (#mspRxBuf < mspRxSize) do
        mspRxBuf[#mspRxBuf + 1] = payload[idx]
        mspRxCRC = bit32.bxor(mspRxCRC, payload[idx])
        idx = idx + 1
    end
    if idx > maxRxBufferSize then
        --wf.print("  mspReceivedReply:  payload continues into next frame.")
        -- Store the last sequence number so we can start there on the next continuation payload
        mspRemoteSeq = seq
        return false
    end
    mspStarted = false
    -- check CRC
    if mspRxCRC ~= payload[idx] and version == 0 then
        --wf.print("  mspReceivedReply:  payload checksum incorrect, message failed!")
        --wf.print("    Calculated mspRxCRC:  0x"..string.format("%X", mspRxCRC))
        --wf.print("    CRC from payload:     0x"..string.format("%X", payload[idx]))
        return nil
    end
    --wf.print("  Got reply for cmd "..mspRxReq)
    return true
end

local function mspPollReply()
    local startTime = wf.clock()
    while wf.clock() - startTime < 0.005 do
        local mspData = mspPoll()
        if mspData == nil then
            return
        end
        if mspReceivedReply(mspData) then
            mspLastReq = 0
            return mspRxReq, mspRxBuf, mspRxError
        end
    end
end

local function mspClearRxTxBufs()
    mspTxBuf = {}
    while mspPoll() do end
end

return mspSendRequest, mspProcessTxQ, mspPollReply, mspClearRxTxBufs
