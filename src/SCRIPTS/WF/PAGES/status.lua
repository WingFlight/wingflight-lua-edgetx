...
local mspDataflash = wf.useApi("mspDataflash")
local mspSetProfile = wf.useApi("mspSetProfile.lua")
...
fields[3] = { t = "Real-time load",        x = x,              y = incY(lineSpacing), sp = x + sp, data = { value = 0, scale = 10, unit = wf.units.percentage }, readOnly = true }
fields[4] = { t = "CPU load",              x = x,              y = incY(lineSpacing), sp = x + sp, data = { value = 0, scale = 10, unit = wf.units.percentage }, readOnly = true }
...
local function armingDisableFlagsToString(flags)
    ...
    for i = 0, 26 do
        if bit32.band(flags, bit32.lshift(1, i)) ~= 0 then
            ...
            if i == 5 then t = t .. "Reserved" end -- was heli governor; bit is never set on this firmware
...
return {
    ...
    timer = function(self)
        if wf.mspQueue:isProcessed() then
            if not editing then
                mspStatus.getStatus(self.onProcessedMspStatus, self)
            end
    ...
    onProcessedMspStatus = function(self, status)
        ...
        if not editing and (fields[1].data.value ~= fcStatus.profile or fields[2].data.value ~= fcStatus.rateProfile) then
            ...
            wf.onPageReady(self) -- force page redraw (important for ui_lvgl)
        end
