...
local endEscEditing = function(field, page)
    ...
    wf.useApi("mspEscBlheliS").read(receivedEscParameters, page)
end

receivedEscParameters = function(page, data)
    escParameters = data
    clearForm(page)
    page.labels, page.fields = wf.executeScript("PAGES/esc_blhelis_form", escParameters, escCount, selectedEsc, endEscEditing)
    page.readOnly = false
    wf.onPageReady(page)
end
...
local page = {
    read = function(self)
        ...
        wf.useApi("mspEscBlheliS").read(receivedEscParameters, self)
    end,
    write = function(self)
        clearForm(self)
        wf.useApi("mspEscBlheliS").write(escParameters)
        wf.useApi("mspEscBlheliS").write(escParameters) -- sometimes writing once won't work, especially after clearEscSelection has been called.
        escParameters = nil
        wf.useApi("mspEscBlheliS").read(receivedEscParameters, self)
    end,
...
page.labels, page.fields = wf.executeScript("PAGES/esc_blhelis_form", nil)
