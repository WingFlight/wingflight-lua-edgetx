...
local escParameters = wf.useApi(mspEscFlyrotor).getDefaults()
...
return {
    read = function(self)
        wf.useApi(mspEscFlyrotor).read(receivedEscParameters, self, escParameters)
    end,
    write = function(self)
        wf.useApi(mspEscFlyrotor).write(escParameters)
