...
local function loadTable(filename)
    local func, err = wf.loadScript(filename)
    if not func then return nil, err end
    return func()
end

local function loadSettings()
    local settings, loadErr = loadTable(wf.baseDir .. "settings.lua") or {}
    --if loadErr then wf.print(loadErr) end
...
local function saveSettings(settings)
    if settings:isDirty() then
        ...
        local saveErr = saveTable(originalTable, wf.baseDir .."settings.lua")
        --if saveErr then wf.print(saveErr) end
    end
end
