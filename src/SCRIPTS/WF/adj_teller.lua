...
if not wf then assert(loadScript("wf.lua"))() end
...
local function run()
    ...
    if timeExitTool then
        -- just show message
        if wf.clock() > timeExitTool then return 2 end
        return 0
    end

    if timeLastChange and wf.clock() - timeLastChange > 1 then
        timeLastChange = nil
        ...
        if adjfuncIdChanged then
            ...
            if adjfunction ~= nil then
                ...
                for i = 1, #adjfunction.wavs do
                    ...
                    playFile(wf.baseDir.."SOUNDS/"..value..".wav")
                end
            end
        end
